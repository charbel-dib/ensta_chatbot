(function () {
  ...
  const rawTitle = (script.getAttribute("data-title") || "Assistant").trim();
  const position = script.getAttribute("data-position") || "right";
  const primary = script.getAttribute("data-primary") || "#111111";

  // (optionnel) cohérence avec chat.html: forcer "phase de test"
  const title = rawTitle.toLowerCase().includes("phase de test")
    ? rawTitle
    : `${rawTitle} (phase de test)`;
  ...
  dot.style.cssText = "width:10px;height:10px;border-radius:999px;background:#35d07f;";
